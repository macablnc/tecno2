...
function draw() {
  ...
  if (capaActual === "N") {
    manejarManchas(manchasN, ManchaN, manchaN, 250, 400);
  } else if (capaActual === "L") {
    manejarManchas(manchasLineas, Linea, lineas,80, 100);
...
function manejarManchas(manchas, ClaseMancha, imagenes, minSize, maxSize) {
  ...
  if (manchas.length < limiteImagenes) {
    if (manchas.length === 0 || tiempoDentroCapa === 0) {
      ...
      let x = random(0, width-5); // Generar posición aleatoria sin límites
      let y = random(0, height-5); // Generar posición aleatoria sin límites
      let nuevaMancha = new ClaseMancha(imagenes[i], x, y, w, h);
